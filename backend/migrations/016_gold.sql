...
--     FRED's keyless GOLDAMGBD228NLBM (sources/fredGold.ts). This is
--     what all history-based math (w1/y1/y5/y10 changes, trend,
--     momentum, volatility) reads. GoldAPI's live tick is NEVER
--     written here.
...
--     falling back to gold_prices' newest FRED close otherwise.
...
create table if not exists gold_prices (
  ...
  source     text not null,             -- 'fred' (only source today)
...
create table if not exists gold_snapshots (
  ...
  price_source   text not null,             -- 'goldapi' (live) | 'fred' (fallback)
