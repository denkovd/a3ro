"use client";
/* ────────────────────────────────────────────────────────────────
   A3RO Intelligence — BTC Tracker · full experience
   Route-only: /Projects/BTC-Tracker. Homepage never imports this.
   Same globe substrate as Oil (dots, bake, camera); orange theme;
   venues + mining + liquidity corridors. Phase 0: honest static /
   connecting states — no invented netflow numbers.
──────────────────────────────────────────────────────────────── */
...
import { FLOW_ROUTES, TERTIARY_PTS, type FlowTier } from "./btc/flowRoutes";
import {
  VENUES,
  MINING_REGIONS,
  MINING_SOURCE,
  type MiningLayerMode,
  type Venue,
  type MiningRegion,
} from "./btc/nodes";
...
export default function BtcTrackerCore({
  ...
  const [layers, setLayers] = useState<{
    flows: boolean;
    venues: boolean;
    mining: MiningLayerMode;
  }>({ flows: true, venues: true, mining: "off" });
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (const c of corridors) {
        ...
        ctx.strokeStyle = ORG(major ? 0.4 : secondary ? 0.3 : 0.2);
        ...
        if (!reducedRef.current && rev >= 1) {
          ...
          const len = 0.1;
          const passes = major
            ? ([[3.4, ORG(0.12)], [1.6, ORG_HI(0.85)]] as const)
            : secondary
              ? ([[2.6, ORG(0.07)], [1.1, ORG(0.65)]] as const)
              : ([[2.6, ORG(0.07)], [1.1, ORG(0.5)]] as const);
      ...
      /* mining overlay */
      const mMode = layersRef.current.mining;
      if (mMode !== "off") {
        const rScale = Math.min(1.25, Math.max(0.8, R / 260));
        const sorted = [...MINING_REGIONS]
          .filter((m) => m.id !== "other")
          .sort((a, b) => b.hashrateShare - a.hashrateShare);
        ctx.font = '500 8px "JetBrains Mono", ui-monospace, monospace';
        for (const [rankIdx, m] of sorted.entries()) {
          ...
          const pct = m.hashrateShare * 100;
          const r = (3 + Math.sqrt(pct) * 1.6) * rScale;
          ctx.strokeStyle = ORG(0.5 * alpha);
          ctx.lineWidth = 1;
          ctx.beginPath();
          ctx.arc(sx, sy, r, 0, Math.PI * 2);
          ctx.stroke();
          ctx.fillStyle = ORG(0.06 * alpha);
          ctx.fill();
          ctx.fillStyle = ORG(0.7 * alpha);
          ctx.beginPath();
          ...
          if (isSel) {
            const b = r + 6;
            ctx.strokeStyle = ORG(0.85 * alpha);
            for (const [dx, dy] of [
              [-1, -1],
              [1, -1],
              [1, 1],
          ...
          const label =
            mMode === "share"
              ? `${m.name} · ${pct.toFixed(0)}%`
              : `${m.name} · SHARE ${pct.toFixed(0)}%`;
      ...
      if (layersRef.current.venues) {
        ...
        for (const v of VENUES) {
          ...
          if (isSel || isHover) {
            const b = 12;
            ctx.strokeStyle = ORG((isSel ? 0.9 : 0.5) * fade);
            ctx.lineWidth = 1;
            for (const [dx, dy] of [
              [-1, -1],
              [1, -1],
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (s.dragging) {
      ...
      const opensPanel = hit != null && (hit.type === "venue" || hit.type === "mining");
  ...
  const onPointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (s.movedPx < 6) {
      ...
      if (hit) {
        if (hit.type === "venue") focusVenue(hit.v);
        else if (hit.type === "mining") focusMining(hit.m);
      }
    }
  };
  ...
  const selVenue = VENUES.find((v) => v.id === selected) ?? null;
  const selMine = selected?.startsWith("mine:")
    ? MINING_REGIONS.find((m) => m.id === selected.slice(5)) ?? null
    : null;
  const hasFocus = selVenue !== null || selMine !== null;

  const hoverCard = (() => {
    if (!hoverId) return null;
    if (hoverId.startsWith("mine:")) {
      const m = MINING_REGIONS.find((x) => x.id === hoverId.slice(5));
      if (!m) return null;
      return {
        title: m.name,
        lines: [
          `HASHRATE SHARE ~${(m.hashrateShare * 100).toFixed(0)}% · ${m.asOf}`,
    ...
    const v = VENUES.find((x) => x.id === hoverId);
    if (!v) return null;
    return {
      title: v.title,
      lines: [
    ...
    <div ref={wrapRef} className={`relative h-full w-full ${className}`}>
      <div ref={stageRef} className="absolute inset-0">
        <canvas
          ...
          onWheel={onWheel}
          aria-label="Bitcoin location and flow globe"
        />
      </div>
        ...
        <AnimatePresence>
          {layersExpanded && (
            ...
            >
              {(
                [
                  ["flows", "Liquidity flows", layers.flows],
                  ["venues", "Venues / ETFs", layers.venues],
                ] as const
              ).map(([key, label, on]) => (
                <button
                  key={key}
                ...
                >
                  <span>{label}</span>
                  <span style={{ color: on ? ORANGE_CSS : "var(--ink-3)" }}>
                    {on ? "ON" : "OFF"}
                  </span>
                </button>
              ))}
              <button
                ...
                onClick={() =>
                  setLayers((L) => {
                    if (L.mining !== "off") {
                      lastMiningRef.current = L.mining;
                      return { ...L, mining: "off" };
                    }
                    return { ...L, mining: lastMiningRef.current };
                  })
                }
                className="flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.18em] text-[var(--ink-2)]"
              >
                <span>Mining regions</span>
                <span
                  style={{
                    color: layers.mining !== "off" ? ORANGE_CSS : "var(--ink-3)",
                  }}
                >
                  {layers.mining === "off" ? "OFF" : layers.mining.toUpperCase()}
                </span>
              </button>
              {layers.mining !== "off" && (
                <div className="ml-1 flex gap-2 border-l border-[var(--line)] pl-2">
                  {(["share", "hashrate"] as const).map((mode) => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => {
                        lastMiningRef.current = mode;
                        setLayers((L) => ({ ...L, mining: mode }));
                      }}
                      className="font-mono text-[8px] uppercase tracking-[0.16em]"
                      style={{
                        color: layers.mining === mode ? ORANGE_CSS : "var(--ink-3)",
                      }}
                    >
                      {mode}
                    </button>
      ...
      {!touched && (
        <p className="pointer-events-none absolute bottom-4 left-1/2 z-10 -translate-x-1/2 font-mono text-[9px] uppercase tracking-[0.22em] text-[var(--ink-3)] md:bottom-6">
          Drag to rotate · scroll to zoom · click a venue
        </p>
      )}
      ...
      >
        <div className="border-b border-[var(--line)] px-4 py-3">
          ...
          <h3 className="mt-1 text-sm font-medium text-[var(--ink)]">
            Known stock · liquidity flow
          </h3>
          <p className="mt-2 text-[11px] leading-relaxed text-[var(--ink-2)]">
            Where known Bitcoin concentrates (exchanges, ETFs, mining regions)
            and the illustrative paths liquidity takes between hubs. Live
            reserve and netflow metrics connect as free feeds are verified —
            never invented.
          </p>
        </div>
            ...
            {VENUES.map((v) => (
              <li key={v.id}>
                <button
                  type="button"
                  onClick={() =>
                    selected === v.id ? closePanel() : focusVenue(v)
                  }
                  ...
                  {selMine && (
                    <>
                      ...
                      <p className="mt-3 font-mono text-2xl text-[var(--ink)]">
                        ~{(selMine.hashrateShare * 100).toFixed(0)}%
                      </p>
                      <p className="mt-1 text-[11px] text-[var(--ink-3)]">
                        Estimated hashrate share
                      </p>
                      <p className="mt-3 text-[11px] leading-relaxed text-[var(--ink-2)]">
                        Supply-side geography — where hashpower concentrates,
                        not where coins are stored. {MINING_SOURCE}.
                      </p>
                    </>
                  )}
                </div>
