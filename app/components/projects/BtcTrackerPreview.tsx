"use client";
/* ────────────────────────────────────────────────────────────────
   BTC Tracker — lightweight teaser globe
   Landing-safe: low detail, no interaction, orange theme.
   Heavy engine lives in BtcTrackerCore (route-only).
──────────────────────────────────────────────────────────────── */
...
import { FLOW_ROUTES } from "./btc/flowRoutes";
import { PRIMARY_VENUES } from "./btc/nodes";

const MAJOR = FLOW_ROUTES.find((r) => r.id === "asia_us_spot")!;

let ARTERY: { samples: Float32Array; n: number } | null = null;

export default function BtcTrackerPreview({
  className = "",
  labels = true,
  initialView,
  viewRef,
  ...
  viewRef?: MutableRefObject<BTView | null>;
}) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (labels) {
        ...
        const marks = PRIMARY_VENUES.map((v, i) => ({
          label: v.label,
          sub: v.status === "connecting" ? "CONNECTING" : "WATCHLIST",
          lon: v.lon,
          lat: v.lat,
        ...
        for (const m of marks) {
          ...
          if (!reducedRef.current) {
            ...
            ctx.strokeStyle = ORG(0.3 * (1 - pt) * fade);
            ctx.beginPath();
